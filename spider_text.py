...
def get_content(num):
    ...
    for page in range(1, int(num)):
        ...
        # pattern = re.compile(r'<div class="j-r-list-c-desc">.*?<a .*?>\s+(.*?)\s+</a>.*?</div>')
        # dr = re.compile(r'<[^>]+>', re.S)
        # result = dr.sub('', pattern)
        # result = re.findall(pattern, html)
        result = re.findall(r'<div class="j-r-list-c-desc">.*?<a .*?>(.*?)</a>.*?</div>',html,re.S)
...
def save(num):
    ...
    with open('a.txt', 'w', encoding='utf-8') as f:
        ...
        # 和上面去掉<br />类似
        for each in text:
            if '<br />' and '<a href='and '</a>' in each:
